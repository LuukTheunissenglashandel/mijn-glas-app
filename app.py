...
# NIEUW: Bulk actie menu
st.markdown("---")
b1, b2, b3 = st.columns([2, 2, 5], vertical_alignment="bottom")
with b1:
    bulk_loc = st.selectbox("Nieuwe locatie voor selectie:", LOCATIE_OPTIES)
with b2:
    bulk_button = st.button("📍 Locatie Bulk Wijzigen", use_container_width=True)
...
# Volgorde aanpassen: Selecteer nu vooraan!
volgorde = ["Selecteer", "Locatie", "Aantal", "Breedte", "Hoogte", "Order", "Uit voorraad_bool", "Omschrijving", "Spouw", "ID"]
view_df = view_df[volgorde]

edited_df = st.data_editor(
    view_df,
    column_config={
        "Selecteer": st.column_config.CheckboxColumn("S", width="small", help="Vink aan voor bulk wijziging"),
        ...
        "Uit voorraad_bool": st.column_config.CheckboxColumn("✅ Uit", width="small"),
...
# A. Check op Bulk Knop
if bulk_button:
    # Kijk welke rijen in de editor zijn aangevinkt
    geselecteerde_ids = edited_df[edited_df["Selecteer"] == True]["ID"].tolist()
    
    if geselecteerde_ids:
        # Update alleen de locatie voor deze specifieke ID's
        st.session_state.mijn_data.loc[st.session_state.mijn_data["ID"].isin(geselecteerde_ids), "Locatie"] = bulk_loc
        sla_data_op(st.session_state.mijn_data)
        st.success(f"✅ Locatie gewijzigd naar {bulk_loc} voor {len(geselecteerde_ids)} rijen!")
        st.rerun()
    else:
        st.warning("⚠️ Vink eerst de rijen aan die je wilt wijzigen in de kolom 'S'.")

# B. Check op individuele wijzigingen (zoals handmatig locatie aanpassen of vinkje 'Uit voorraad')
    ...
    # Update op basis van ID om fouten bij sortering/filtering te voorkomen
